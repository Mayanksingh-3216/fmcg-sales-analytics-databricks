...
base_path = f's3://sportsbar-db-15-03-2025/{data_source}'
...
df_fixed = df.withColumn("order_qty", F.col("order_qty").cast("bigint"))
df_fixed.write\
  .format("delta") \
...
# DBTITLE 1,Write Delta Table
df_staging = df.withColumn("order_qty", F.col("order_qty").cast("int"))
df_staging.write\
 .format("delta") \
 .option("delta.enableChangeDataFeed", "true") \
 .mode("overwrite") \
